...
class ImageCopywriter:
    ...
    def _create_copy_sync(self, image_bytes: bytes, mime_type: str) -> ProductCopy:
        prompt = (
            ...
            "Если в принте есть короткая главная надпись до трех слов (например, 'Sportsmenler'), предпочти ее. "
            "Определи целевую аудиторию принта (target_audience): "
            "'men' - для спортивных, борцовских, мужских, дерзких, мемных или стритвир принтов; "
            "'women' - для женственных, цветочных, эстетичных принтов; "
            "'unisex' - для универсальных нейтральных принтов. "
            "Создай один короткий продающий хук из 8-14 слов, строго в стиле самого принта: "
            "для спортивных/борцовских/мемных принтов напиши дерзкую, энергичную или ироничную фразу (НЕ пиши шаблонные клише про 'обнять весь мир' или 'поделиться теплом', если на принт не милая романтика). "
            "Пиши на "
            f"{self.language} языке, естественно смешивай русский и английский если это есть в принте. "
            "Избегай пустых клише вроде «стильный образ» или «идеально на каждый день». Пиши как человек. "
            "В поле theme_hashtag верни одно слово без пробелов. Не включай тип вещи в design_name."
        )
        response = None
