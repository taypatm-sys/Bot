...
```text
Фото на модели: gemini-3.1-flash-image, 1K, 4:5
```
...
- Создание трех реалистичных фотографий 4:5 из готового макета.
...
Модель `gemini-3.1-flash-image` не имеет бесплатного API-тарифа. В проекте
...
Стандартная серия создает три изображения 1K и по текущему тарифу стоит около
0.20 доллара. Актуальная цена указана на странице:
